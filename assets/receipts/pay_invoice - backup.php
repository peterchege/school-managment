...
$pdf -> Cell(46, 4, $current_parent['phone'], 0, 1);
$pdf -> Cell(130, 4, '', 0);
...
while($payments = mysqli_fetch_assoc($payments_set)){
    $total_term_amount += $payments["amount"];
    $total_term_paid += $payments["paid"];
}
...
while($total_payments = mysqli_fetch_assoc($total_payments_set)){
    $total_amount += $total_payments["amount"];
...
$pdf -> Cell(15, 5, $total_term_balance, 0, 1);
